...
**به علاوه استفاده از تابع این ویژگی های مثبت رو داره:**
...
با این تفاوت که تابع ها در برنامه نویسی میتواند هیچ ورودی نداشته یا چند ورودی داشته باشند.
...
### تابع فاکتوریل
در ادامه تابع فاکتوریل تعریف شد:
```c
int fact(int x)
{
  int r = 1;
...
کتابخانه ای به نام 
`assert`
وجود دارد که کار تست کردن تابع های نوشته شده را آسان میکند.

به صورت زیر میتوان کتابخانه را به برنامه معرفی کرد:
```c
#include <assert.h>
```
...
## پیاده سازی تابع sin
در ادامه  تابعی پیاده سازی کردیم که مقدار 
`sin(x)`
